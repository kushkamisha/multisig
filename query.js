...
module.exports = async (callback) => {
  try {
    ...
    const data = getEncodedTransfer({ instance: tokenInstance, to: destination, value: '10' });

    console.log({ multisigAddr, tokenAddr, destination, value, nonce, data });

    await signAndSendTransfer({ data, from: senderAddr, to: tokenAddr, prKey: senderPrKey });
    ...
    // await multisig.execute(tokenAddr /*destination*/, value, data, [r], [s], [v]);
